...
stopFlag=0;
count=0;
while stopFlag==0 && count<40 || count<=Ndz      
    % Points falling    
%     vmVec=zeros(nPoints,nDim);
%     vmVnorm=zeros(nPoints,nDim)+err;
    ...
    bdLoRep=repmat(bdLim(1,1:nDim-1),length(activePList),1,nFlys);%%boundary Check
    bdUpRep=repmat(bdLim(2,1:nDim-1),length(activePList),1,nFlys);
    bdOutUpIndex=find(posRep(activePList,1:nDim-1,:)>bdUpRep);
    bdOutLoIndex=find(posRep(activePList,1:nDim-1,:)<bdLoRep);
    if ~isempty(bdUpRep)
        bdUpRepNd=linerIndex2Nd(bdOutUpIndex,[length(activePList),nDim,nFlys]);
        posRep(activePList(bdUpRepNd(:,1)),1:nDim-1,bdUpRepNd(:,3))=bdUpRep(bdUpRepNd(:,1),1:nDim-1,bdUpRepNd(:,3));
    end
    if ~isempty(bdLoRep)
        bdLoRepNd=linerIndex2Nd(bdOutLoIndex,[length(activePList),nDim,nFlys]);
        posRep(activePList(bdLoRepNd(:,1)),1:nDim-1,bdLoRepNd(:,3))=bdLoRep(bdLoRepNd(:,1),1:nDim-1,bdLoRepNd(:,3));
    end
    posRepActiCell_z=mat2cell(posRep(activePList,1:nDim-1,:),length(activePList),ones(1,nDim-1),nFlys);
    posZFlys=reshape(zFunc(posRepActiCell_z{:}),length(activePList),nFlys);
    ...
    xyMinsPos=[];
    for index=1:length(activePList)
